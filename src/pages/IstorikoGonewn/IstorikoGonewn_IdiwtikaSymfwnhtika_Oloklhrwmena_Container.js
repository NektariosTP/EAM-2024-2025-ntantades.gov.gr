...
import "./IstorikoGonewn_IdiwtikaSymfwnhtika_Oloklhrwmena_Container.css";

export default function IstorikoGonewn_IdiwtikaSymfwnhtika_Oloklhrwmena_Container() {
  ...
  const getCompletedContracts = () => {
    ...
    const savedContracts = JSON.parse(localStorage.getItem("completedContracts")) || [];
    ...
    localStorage.setItem("completedContracts", JSON.stringify(uniqueContracts));
          ...
          {currentContracts.map((contract, index) => (
            <div key={index} className="oloklhrwmena-item">
              ...
              <div className="oloklhrwmena-content">
                <p>
                  Όνομα Νταντάς: <span className="oloklhrwmena-nanny-name">{contract.name}</span>
                </p>
                <p >
              ...
              {contract.type && (
                  ...
                  <button className="edit-button" onClick={handleAnanewsh}>Ανανέωση Ιδιωτικού Συμφωνητικού</button>
                  <button className="cancel-button">Λήξη Συνεργασίας</button>
              </div>
                )}
