...
export default function IstorikoGonewn_Rantevou_Akyrwmena_Container() {
  ...
  const [allAppointments, setAllAppointments] = useState(() => {
    const savedAppointments = JSON.parse(localStorage.getItem("canceledAppointments"));

    if (savedAppointments && savedAppointments.length > 0) {
      const uniqueAppointments = removeDuplicates(savedAppointments);
      localStorage.setItem("canceledAppointments", JSON.stringify(uniqueAppointments));
      ...
      localStorage.setItem("canceledAppointments", JSON.stringify(uniqueInitialAppointments));
