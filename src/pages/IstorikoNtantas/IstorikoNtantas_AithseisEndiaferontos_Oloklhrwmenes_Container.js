...
import "../IstorikoGonewn/IstorikoGonewn_AithseisEndiaferontos_Oloklhrwmenes_Container.css";

export default function IstorikoNtantas_AithseisEndiaferontos_Oloklhrwmenes_Container() {
  ...
  const [allRequests, setAllRequests] = useState(() => {
    const savedRequests = JSON.parse(localStorage.getItem("completedRequests_ntanta")) || [];
...
useEffect(() => {
  localStorage.setItem("completedRequests_ntanta", JSON.stringify(allRequests));
}, [allRequests]);
...
const [filteredRequests, setFilteredRequests] = useState(allRequests);
  ...
  const deleteRequest = (id) => {
    ...
    localStorage.setItem("completedRequests_ntanta", JSON.stringify(updatedLocalStorage));
