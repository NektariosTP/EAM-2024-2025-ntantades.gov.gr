...
export default function IstorikoNtantas_Rantevou_Prosexws_Container() {
  const initialAppointments = [
    ...
    },
    {
        ...
        nannyName: "Μαρία Παπαχρήστου",
        type: "Δια ζώσης",
        time: "8:00 μμ",
        location: " Λεωφόρος Στρατάρχου Αλέξανδρου Παπάγου 100,Ζωγράφου 15772 ",
        approved: false,
      },
      {
        date: "26/05/2024",
  ...
  const [currentFilter, setCurrentFilter] = useState("Ημερομηνία(Αύξουσα)");
   const [allAppointments, setAllAppointments] = useState(() => {
    const savedAppointments = JSON.parse(localStorage.getItem("upcomingAppointments_ntanta"));
    ...
    if (savedAppointments && savedAppointments.length > 0) {
        ...
        localStorage.setItem("upcomingAppointments_ntanta", JSON.stringify(sortedInitialAppointments));
...
useEffect(() => {
    localStorage.setItem("upcomingAppointments_ntanta", JSON.stringify(allAppointments));
}, [allAppointments]);
...
const filterAnamonh = () => {
  ...
  const [canceledAppointments, setCanceledAppointments] = useState(() => {
    const savedCanceledAppointments = JSON.parse(localStorage.getItem("canceledAppointments_ntanta"));
  ...
  const cancelAppointment = (index) => {
    ...
    setCanceledAppointments((prev) => {
        ...
        localStorage.setItem("canceledAppointments_ntanta", JSON.stringify(updatedCanceledAppointments));
    ...
    const allAppointments = JSON.parse(localStorage.getItem("upcomingAppointments_ntanta")) || [];
    ...
    localStorage.setItem("upcomingAppointments_ntanta", JSON.stringify(updatedAllAppointments));
...
const confirmAppointment = (index) => {
    ...
    updatedAppointments[index].status = "Προγραμματισμένο";
    updatedAppointments[index].approved = true;

    const confirmedAppointment = updatedAppointments[index];
    localStorage.setItem("latestConfirmedAppointment", JSON.stringify(confirmedAppointment));
  
    setAllAppointments(updatedAppointments);
    localStorage.setItem("upcomingAppointments_ntanta", JSON.stringify(updatedAppointments));
  
    

    // Redirect to confirmation screen
    window.location.href = "/istorikoo/rantevou/prosexws/epivevaiwsh"; // Adjust URL as needed
  };
                ...
                {!appointment.approved ? (
                  <>
                    <button
                      ...
                      onClick={() => confirmAppointment(index)}
                    >
                      Επιβεβαίωση Ραντεβού
                    </button>
                    <button
                      className="edit-buttonn"
                      onClick={() => window.location.href = "/Ntantades/Profil/Rantevou/Epeksergasia"}
                    >
