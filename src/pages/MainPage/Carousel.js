...
function Carousel() {
    ...
    useEffect(() => {
        // Shuffle the reviews and take the first 5
            ...
            <div className="carousel-container">
                ...
                <div className="review-cards">
                    {displayedReviews.length > 0 ? (
                        [0, 1, 2].map((offset) => {
                            const index = (currentIndex + offset) % displayedReviews.length;
                            const review_view = displayedReviews[index];
                            return (
                                <div
                                    key={review_view.id}
                                    className="review-card"
                                    onClick={() => handleReviewClick(review_view.id)}
                                >
                                    <img
                                        className="avatar"
                                        src={`/nanny${review_view.id}.jpg`}
                                        alt={`Avatar of ${review_view.name}`}
                                    />
                                    <h3 className="review-name-search1"> {review_view.name}</h3>
                                    <p className="location">{review_view.location}</p>
                                    <p className="rating">⭐ {review_view.rating}</p>
                                </div>
                            );
                        })
                    ) : (
                        <p>Loading reviews...</p> // Fallback UI
                    )}
                </div>
