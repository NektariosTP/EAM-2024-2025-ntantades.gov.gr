...
import "./NannyEditForm2.css";
import { useNavigate } from "react-router-dom";

function NannyEditForm2() {
  ...
  useEffect(() => {
    ...
    const savedData = localStorage.getItem("nannyFormData2");
    if (savedData) {
  ...
  const handleChange = (e) => {
    ...
    localStorage.setItem("nannyFormData2", JSON.stringify(updatedFormData2));
  ...
  const handleClearForm = () => {
    ...
    setErrors({});
    setTouched({});
    localStorage.removeItem("nannyFormData2");
  };
  ...
  return (
    <form className="form-container" onSubmit={handleSubmit}>
      ...
      <label className="label-nanny">IBAN Τραπεζικού Λογαριασμού (*)</label>
      <input
        type="text"
        name="iban"
        value={formData.iban}
        onChange={handleChange}
        onBlur={handleBlur}
        className={errors.iban ? "input-nanny" : ""}
      />
