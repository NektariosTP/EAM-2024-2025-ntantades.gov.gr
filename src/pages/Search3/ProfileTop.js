    ...
    return (
        <div className="profile-container" style={{ backgroundImage: 'url(/profile_background.jpg)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
            <div className="profile-content">
                <img className="profile-image-search3" src={`/nanny${nanny.id}.jpg`} alt={`nanny${nanny.id}`} />
                <div className="profile-info">
                    <p className="nanny-name-profile"> {nanny.name}</p>
                    <p className="profile-details">{nanny.age} ετών | {nanny.years_exp} Χρόνια Εμπειρίας</p>
                    <p className="profile-program">{nanny.degree}</p>
                    <div className="profile-rating">
                        <span className="rating-score">{nanny.rating}</span>
                        <div className="stars-profile">
                            {Array.from({ length: 5 }).map((_, index) => (
                                <span key={index}>{index < Math.round(nanny.rating) ? '★' : '☆'}</span>
                            ))}
                        </div>
                        <span className="rating-count">({nanny.review_count})</span>
                    </div>
                </div>
            </div>
        </div>
